...
def add_user(id_tg: int, username: str = None):
    with Database() as curs:
        if not check_user(id_tg):
            if username:
                _SQL = f'INSERT INTO users (id_tg, username, isActive) \
                         VALUES ({id_tg}, $${username}$$, false);'
            else:
                _SQL = f'INSERT INTO users (id_tg, isActive) \
                         VALUES ({id_tg}, false);'
            curs.execute(_SQL)
...
def set_user_params(id_tg: int, data: dict):
    with Database() as curs:
        _SQL = f"UPDATE users SET params = '{json.dumps(data)}' \
                 WHERE id_tg = {id_tg};"
        curs.execute(_SQL)
...
def set_user_status(id_tg: int, is_active: bool):
    with Database() as curs:
        _SQL = f"UPDATE users SET isActive = {is_active} \
                 WHERE id_tg = {id_tg};"
        curs.execute(_SQL)
