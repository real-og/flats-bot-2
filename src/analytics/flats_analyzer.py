...
def analyze_minsk_advertisements(csv_file):
    ...
    affordable_non_minsk_df = df[(df['town'] == 'Минск') & (df['cost'] <= 2000) & (df['rooms_amount'] == '1')]

    daily_statistics = affordable_non_minsk_df.groupby(affordable_non_minsk_df['date'].dt.date).agg(
